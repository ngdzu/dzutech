...
const isPlaceholder = (value) => {
  ...
  return (
    ...
    [
      'changeme',
      'please-change-me',
      'please-change-me-with-at-least-32-characters',
      'placeholder',
      'example@example.com',
    ].includes(normalized)
  );
...
const requiredEnv = [
  {
    ...
    validate: (value) => {
      if (isPlaceholder(value))
        return 'DATABASE_URL must be a non-empty, production-ready connection string.';
  ...
  },
  {
    ...
    validate: (value) => {
      if (isPlaceholder(value))
        return 'ADMIN_EMAIL must be set to the administrator email address.';
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(value ?? '')) {
  ...
  },
  {
    ...
    validate: (value) => {
      if (
        lowerTrim(process.env.DATABASE_URL)?.includes('localhost') ||
        lowerTrim(process.env.DATABASE_URL)?.includes('127.0.0.1')
      ) {
