...
if (Object.keys(coverage).length === 0) {
  console.error(
    'coverage report not found; run tests with coverage before running this check (generate coverage/coverage-final.json and/or server/coverage/coverage-final.json)',
  );
  process.exit(2);
}
...
const excludeExactPaths = new Set(['server/src/index.ts', 'src/App.tsx']);
...
if (failures.length) {
  console.error(
    '\nCoverage check failed: the following staged files are below the threshold of ' +
      THRESHOLD +
      '%:',
  );
  failures.forEach((f) =>
    console.error(` - ${f.file}: ${f.pct}% ${f.reason ? '(' + f.reason + ')' : ''}`),
  );
