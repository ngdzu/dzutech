  ...
  return {
    getContent: vi.fn(async () => ({
      ...
      profile: {
        ...
        focusAreas: { value: '', enabled: true },
        focusAreas2: undefined,
      },
      sections: {
        contact: { description: '' },
...
describe('index API routes (smoke/branch coverage)', () => {
  ...
  it('GET /api/content success and failure', async () => {
    ...
    // success
    repoAny.getContent.mockImplementationOnce(async () => ({
      posts: [],
      profile: {},
      sections: {},
      experiences: [],
  ...
  it('POST /api/auth/login cases', async () => {
    ...
    // wrong email
    res = await request((globalThis as any).serverApp)
      .post('/api/auth/login')
      .send({ email: 'bad@example.com', password: 'x' });
    expect(res.status).toBe(401);

    // correct email, bcrypt mocked true -> success
    res = await request((globalThis as any).serverApp)
      .post('/api/auth/login')
      .send({ email: process.env.ADMIN_EMAIL, password: 'x' });
    expect(res.status).toBe(200);
  });
  ...
  it('DELETE /api/posts/:postId missing and not found', async () => {
    ...
    // not found
    repoAny.removePostById.mockImplementationOnce(async () => {
      const e: any = new Error('not');
  ...
  it('PATCH /api/posts/:postId/visibility validation and not found', async () => {
    ...
    const repoAny: any = repo;
    // missing post id
    let res = await request((globalThis as any).serverApp)
    ...
    // invalid hidden body
    res = await request((globalThis as any).serverApp)
      .patch('/api/posts/1/visibility')
      .send({ hidden: 'yes' });
    expect(res.status).toBe(400);

    // not found
    repoAny.setPostHidden.mockImplementationOnce(async () => {
  ...
  it('POST /api/reset success and failure', async () => {
    ...
    repoAny.resetContent.mockImplementationOnce(async () => {
      throw new Error('boom');
    });
  ...
  it('PUT /api/profile validation and save paths', async () => {
    ...
    res = await request((globalThis as any).serverApp)
      .put('/api/profile')
      .send({ highlightsEnabled: true, availability: { value: long } });
    expect(res.status).toBe(422);
  ...
  it('PUT /api/posts validation and save', async () => {
    ...
    res = await request((globalThis as any).serverApp)
      .put('/api/posts')
      .send([{ id: '1', title: 'T', content: 'C', tags: [] }]);
    expect(res.status).toBe(200);
  });
  ...
  it('PUT /api/experiences validation and save', async () => {
    ...
    res = await request((globalThis as any).serverApp)
      .put('/api/experiences')
      .send([{ role: '', company: '', year: '', description: '', achievements: [], stack: [] }]);
    expect(res.status).toBe(422);
    ...
    res = await request((globalThis as any).serverApp)
      .put('/api/experiences')
      .send([
        { role: 'R', company: 'C', year: 'Y', description: 'D', achievements: ['a'], stack: ['s'] },
      ]);
    expect(res.status).toBe(200);
  });
  ...
  it('login sets session and /api/auth/me and logout', async () => {
    // perform login
    const loginRes = await request((globalThis as any).serverApp)
      .post('/api/auth/login')
      .send({ email: process.env.ADMIN_EMAIL, password: 'x' });
    ...
    const meRes = await request((globalThis as any).serverApp)
      .get('/api/auth/me')
      .set('Cookie', cookies);
    ...
    // logout
    const outRes = await request((globalThis as any).serverApp)
