...
describe('repository exports (db interactions and content helpers)', () => {
  it('saveUpload, getUploadById and listUploads work with pool.query', async () => {
    ...
    const saved = await repo.saveUpload({
      key: record.key,
  ...
  it('savePosts normalizes posts and returns normalized array', async () => {
    ...
    const postsIn = [
      { id: '  id1  ', title: '  t  ', content: '  c  ', tags: [' a ', null], hidden: true },
    ];
  ...
  it('removePostById throws when not found and removes when present', async () => {
    ...
    stored.posts = [
      { id: 'p1', title: 't', content: 'c', contentHtml: '', tags: [], hidden: false },
    ];
  ...
  it('setPostHidden throws when not found and updates when present', async () => {
    ...
    await expect(repo.setPostHidden('nope', true)).rejects.toMatchObject({
      code: 'POST_NOT_FOUND',
    });

    stored.posts = [
      { id: 'p2', title: 't', content: 'c', contentHtml: '', tags: [], hidden: false },
    ];
  ...
  it('resetContent writes defaults for all keys', async () => {
    vi.resetModules();
    const writes: any[] = [];
    const writeJson = async (key: string, _value: any) => {
      void _value;
      writes.push(key);
    };
    await vi.doMock('./db.js', async () => ({ writeJson }));
    ...
    expect(writes).toEqual(
      expect.arrayContaining(['site', 'profile', 'experiences', 'posts', 'sections']),
    );
  });
});
