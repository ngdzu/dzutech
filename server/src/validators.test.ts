...
describe('sanitizeLogo', () => {
  ...
  it('throws when data URL prefix does not match type', () => {
    ...
    expect(() => sanitizeLogo({ data: 'not-a-data-url', type })).toThrow(
      'Logo data must be a base64 data URL',
    );
  });
...
describe('validateExperience', () => {
  ...
  it('rejects non-string arrays for achievements', () => {
    ...
    expect(validateExperience(e1, 2)).toBe(
      'experiences[2].achievements must be an array of strings',
    );
  });
});
...
describe('validatePost', () => {
  ...
  it('rejects non-string id', () => {
    expect(validatePost({ ...base, id: 1 as unknown as string }, 0)).toBe(
      'posts[0].id must be a string',
    );
  });

  it('rejects non-boolean hidden', () => {
    expect(validatePost({ ...base, hidden: 'no' as unknown as boolean }, 0)).toBe(
      'posts[0].hidden must be a boolean',
    );
  });
  ...
  it('rejects missing title/content/tags', () => {
    ...
    expect(validatePost({ ...base, tags: 'tag' as unknown as string[] }, 5)).toBe(
      'posts[5].tags must be an array of strings',
    );
  });
});
...
describe('validateSections', () => {
  it('rejects missing contact description', () => {
    expect(validateSections({ contact: { description: '' } } as unknown as SectionsContent)).toBe(
      'sections.contact.description is required',
    );
  });
});
