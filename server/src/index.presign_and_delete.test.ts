...
describe('generatePresignedUrlForKey and admin delete branches', () => {
  it('falls back to proxy path when PUBLIC_S3_ENDPOINT presigning fails', async () => {
    ...
    vi.doMock('./repository.js', () => ({
      getUploadById: vi.fn(async (id: string) =>
        id === 'uuid-10' ? { id: 'uuid-10', filename: 'file.png', key: 'uploads/file.png' } : null,
      ),
    }));
  ...
  it('returns proxy path when S3 endpoint hostname contains minio (no presign)', async () => {
    ...
    vi.doMock('./repository.js', () => ({
      getUploadById: vi.fn(async (id: string) =>
        id === 'uuid-11'
          ? { id: 'uuid-11', filename: 'minio.png', key: 'uploads/minio.png' }
          : null,
      ),
    }));
  ...
  it('admin delete continues when S3 delete fails', async () => {
    ...
    vi.doMock('./repository.js', () => ({
      getUploadById: vi.fn(async () => ({ id: 'del-1', key: 'uploads/secret.png' })),
      deleteUpload,
    }));
    ...
    vi.doMock('@aws-sdk/client-s3', () => ({
      S3Client: class {
        send() {
          const e: any = new Error('S3 delete failed');
          e.name = 'AccessDenied';
          throw e;
        }
      },
