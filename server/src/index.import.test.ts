...
vi.mock('express', () => {
  ...
  (expressFactory as any).json = () => (req: unknown, res: unknown, next: unknown) => {
    if (typeof next === 'function') (next as any)();
  };
  (expressFactory as any).static =
    (dir: unknown) => (req: unknown, res: unknown, next: unknown) => {
      if (typeof next === 'function') (next as any)();
    };
...
vi.mock('express-session', () => ({
  default: () => (req: unknown, res: unknown, next: unknown) => {
    if (typeof next === 'function') (next as any)();
  },
}));
vi.mock('connect-pg-simple', () => ({
  default: (session: any) => {
    ...
    class MockStore extends Base {
      ...
      set(sid: any, sess: any, cb: any) {
        cb && cb();
      }
      destroy(sid: any, cb: any) {
        cb && cb();
      }
    }
    return MockStore;
