...
describe('uploadHandler additional branches', () => {
  it('returns 400 for non-raster detected content (e.g., gif)', async () => {
    ...
    await vi.doMock('./repository.js', async () => ({
      saveUpload: async (u: any) => ({
        id: 'mock-upload-id',
        filename: u.filename,
        mimetype: u.mimetype,
        size: u.size,
    ...
    await vi.doMock('file-type', async () => ({
      fileTypeFromBuffer: async () => ({ mime: 'image/gif' }),
    }));
  ...
  it('rejects SVG by text heuristic when file-type returns undefined', async () => {
    ...
    const req: any = {
      file: { originalname: 's.svg', mimetype: 'image/svg+xml', buffer: svgText },
    };
    const res = createRes();
    await uploadHandler(req, res);
    expect(res.statusCode).toBe(400);
  ...
  it('falls back to original buffer when sharp is unavailable', async () => {
    ...
    const req: any = {
      file: { originalname: 'nof-sharp.png', mimetype: 'image/png', buffer: buf },
    };
    const res = createRes();
    await uploadHandler(req, res);
  ...
  it('on DB save failure returns uploads URL fallback', async () => {
    ...
    await vi.doMock('sharp', async () => ({
      default: (input: unknown) => ({
        resize: () => ({ toBuffer: async () => Buffer.from(input as Buffer) }),
        toBuffer: async () => Buffer.from(input as Buffer),
      }),
    }));
