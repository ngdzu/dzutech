...
describe('presign success and admin uploads list', () => {
  it('returns presigned URL when getSignedUrl succeeds for /photos/:id', async () => {
    ...
    vi.doMock('./repository.js', () => ({
      getUploadById: vi.fn(async (id: string) =>
        id === 'uuid-20' ? { id: 'uuid-20', filename: 'ok.png', key: 'uploads/ok.png' } : null,
      ),
    }));
    vi.doMock('@aws-sdk/s3-request-presigner', () => ({
      getSignedUrl: async () => 'https://signed.example/ok.png',
    }));
  ...
  it('admin GET /api/admin/uploads includes presignedUrl when S3 is configured', async () => {
    ...
    vi.doMock('@aws-sdk/s3-request-presigner', () => ({
      getSignedUrl: async () => 'https://signed.example/r1.png',
    }));
