...
describe('POST /api/uploads', () => {
  ...
  it('accepts an image when authenticated', async () => {
    ...
    // 1x1 transparent PNG
    const pngBase64 =
      'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQMBARc9y7kAAAAASUVORK5CYII=';
    const res = await agent.post('/api/uploads').attach('file', Buffer.from(pngBase64, 'base64'), {
      filename: 'small.png',
  ...
  it('rejects unsupported MIME types', async () => {
    ...
    const res = await agent.post('/api/uploads').attach('file', Buffer.from('hello world'), {
      filename: 'notes.txt',
  ...
  it('rejects unknown content when file-type returns undefined and payload is non-SVG text', async () => {
    ...
    const res = await agent
      .post('/api/uploads')
      .attach('file', Buffer.from('not an image, just text'), {
  ...
  it('rejects SVG content when file-type returns undefined and buffer contains <svg>', async () => {
    ...
    const res = await agent.post('/api/uploads').attach('file', Buffer.from(svg, 'utf8'), {
      filename: 'inline.svg',
