...
const databaseUrl = hasCustomDatabaseUrl
  ? (process.env.DATABASE_URL as string).trim()
...
if (nodeEnv === 'production' && !sslEnabled) {
  console.warn(
    'DB_SSL is not enabled; enable TLS between the API and database for production deployments.',
  );
}
