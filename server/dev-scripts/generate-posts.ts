...
const parseArgs = () => {
  ...
  const out: { count: number; skipDb: boolean; dryRun: boolean } = {
    count: 50,
    skipDb: false,
    dryRun: false,
  };
...
const makeImageUrl = (seed: string, width = 1200, height = 628) =>
  ...
  while (curWords < targetWords - 200 && !(maxWordsCap && curWords >= maxWordsCap)) {
    ...
    parts.push(
      para +
        ' ' +
        (paraIndex % 3 === 0
          ? 'We illustrate with concrete examples and measurements.'
          : 'Focus on correctness first, optimize with data.'),
    );
    paraIndex += 1;
...
export const generateAndSavePosts = async (count = 50, options: GenerateOptions = {}) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const relativePath = path.join(
      'server',
      path.relative(path.resolve(process.cwd(), 'server'), mdPath),
    );
    const meta = {
      id: post.id,
      title: post.title,
  ...
  if (!skipDb) {
    try {
      ...
      const saved = await repo.savePosts(
        toSave as unknown as Array<Partial<GeneratedPost> & Record<string, unknown>>,
      );
