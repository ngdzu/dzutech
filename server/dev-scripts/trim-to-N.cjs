...
function parseKeepArg() {
  const arg = process.argv[2] || process.env.KEEP || '50';
...
function main() {
  ...
  console.log(`Found ${files.length} md files, ${seen.size} unique by hash, deleted ${deletedCount} files, keeping ${newMeta.length} entries in index (target KEEP=${KEEP}).`);
}
