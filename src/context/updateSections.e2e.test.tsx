  ...
  it('calls API and updates context with nested sections', async () => {
    const newSections: SectionsContent = {
      ...
      educations: {
        visible: true,
        items: [
          { institution: 'Test U', degree: 'BS', year: '2012', description: 'Studied tests' },
        ],
      },
    ...
    await waitFor(() =>
      expect(
        (globalThis as unknown as Record<string, unknown>).__mockFetchContent as any,
      ).toHaveBeenCalled(),
    );
    ...
    await waitFor(() =>
      expect(
        (globalThis as unknown as Record<string, unknown>).__mockUpdateSections as any,
      ).toHaveBeenCalledWith(newSections),
    );

    // Ensure provider content was updated and consumer sees new contact description
    await waitFor(() =>
      expect(screen.getByTestId('contact-desc')).toHaveTextContent('Reach out via email'),
    );
  });
});
