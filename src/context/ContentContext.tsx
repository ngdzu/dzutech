...
export const ContentProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const computedTitle =
      siteTitle || [name, role].filter(Boolean).join(' · ') || 'Personal Portfolio';
    ...
    const fallbackDescription =
      'Portfolio website showcasing software engineering work, projects, and contact details.';
    const computedDescription =
      siteDescription || summary || contactDescription || fallbackDescription;
  ...
  const setPostVisibility = useCallback(async (postId: string, hidden: boolean) => {
    try {
      ...
      setError(
        updateError instanceof Error ? updateError.message : 'Failed to update post visibility',
      );
      throw updateError;
    }
  }, []);
