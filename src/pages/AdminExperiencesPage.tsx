...
const AdminExperiencesPage = () => {
  ...
  const [experiencesForm, setExperiencesForm] =
    useState<ExperienceFormEntry[]>(experiencesInitialForm);
  ...
  useEffect(() => {
    setProgrammingLanguagesRaw(
      ((sectionsInitialForm as unknown as SectionsFormState).programmingLanguagesItems ?? []).join(
        ', ',
      ),
    );
  }, [sectionsInitialForm]);
  useEffect(() => {
    setLanguagesSpokenRaw(
      ((sectionsInitialForm as unknown as SectionsFormState).languagesSpokenItems ?? []).join(', '),
    );
  }, [sectionsInitialForm]);

  const updateSectionsFormField = <T extends keyof SectionsFormState>(
    prev: SectionsFormState,
    field: T,
    value: SectionsFormState[T],
  ): SectionsFormState => {
    return { ...prev, [field]: value } as SectionsFormState;
  };

  type ArrayStringField =
    | 'programmingLanguagesItems'
    | 'languagesSpokenItems'
    | 'achievementsItems';

  const handleToggle =
    (field: keyof SectionsFormState & string) => (event: ChangeEvent<HTMLInputElement>) => {
      setSectionsForm((prev) =>
        updateSectionsFormField(
          prev,
          field as keyof SectionsFormState,
          event.target.checked as SectionsFormState[typeof field],
        ),
      );
    };

  const handleEditArrayItem =
    (field: ArrayStringField, index: number) =>
    (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setSectionsForm((prev) => {
        const arr = (prev[field] ?? []) as string[];
        const next = arr.slice();
        next[index] = event.target.value;
        return updateSectionsFormField(
          prev,
          field as keyof SectionsFormState,
          next as SectionsFormState[typeof field],
        );
      });
    };

  const handleEditEducation =
    (index: number, key: keyof SectionsFormState['educationsItems'][number]) =>
    (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      setSectionsForm((prev) => {
        const copy = { ...prev };
  ...
  const handleAddArrayItem =
    (field: keyof SectionsFormState & string, initial: EducationItem | string = '') =>
    () => {
      setSectionsForm((prev) => {
        if (field === 'educationsItems') {
          const items = (
            (prev.educationsItems ?? []) as SectionsFormState['educationsItems']
          ).slice();
          items.push(initial as EducationItem);
          return updateSectionsFormField(
            prev,
            'educationsItems',
            items as SectionsFormState['educationsItems'],
          );
        }
        ...
        return updateSectionsFormField(
          prev,
          field as keyof SectionsFormState,
          [...arr, initial as string] as SectionsFormState[typeof field],
        );
      });
    };
  ...
  const handleRemoveArrayItem = (field: keyof SectionsFormState & string, index: number) => () => {
    setSectionsForm((prev) => {
      ...
      return updateSectionsFormField(
        prev,
        field as keyof SectionsFormState,
        arr as SectionsFormState[typeof field],
      );
    });
  };
  ...
  const handleSectionsSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setExperiencesStatus({
        state: 'error',
        message: err instanceof Error ? err.message : 'Failed to save sections',
      });
    }
  };
  ...
  const handleExperienceTextChange =
    (index: number, field: 'role' | 'company' | 'year' | 'description' | 'location') =>
    (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const handleExperienceAchievementsChange =
  ...
  const handleExperiencesSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (sanitized.length === 0) {
      setExperiencesStatus({
        state: 'error',
        message: 'Add at least one experience before saving',
      });
      return;
    }
    ...
    <div className="min-h-screen bg-night-900 text-slate-100">
      ...
      <main className="mx-auto flex max-w-5xl flex-col gap-6 px-6 py-10">
        <form onSubmit={handleExperiencesSubmit} className="space-y-6">
              ...
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center rounded-full border border-slate-700/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.25em] text-slate-300 transition hover:border-red-400/60 hover:text-red-200"
                  >
                    Remove
                  </button>
                </div>
                ...
                <label className="flex flex-col gap-2">
                  ...
                  <textarea
                    className={`${fieldStyle} min-h-[80px]`}
          ...
          {experiencesStatus.state === 'error' && (
            <p className="text-sm text-red-300">{experiencesStatus.message}</p>
          )}
        ...
        >
          <div className="space-y-2">
            ...
            <p className="text-sm text-slate-400">
              Control the content blocks shown on the experiences page: education, programming
              languages, languages spoken, and achievements.
            </p>
          </div>
          ...
          <div className="space-y-4 rounded-2xl border border-slate-800/70 bg-night-900/50 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Show or hide the dedicated experiences page
                </p>
              </div>
              <label className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-300">
                <input
                  type="checkbox"
                  className="h-4 w-4"
                  checked={sectionsForm.experiencesVisible}
                  onChange={handleToggle('experiencesVisible')}
                />
          ...
          <div className="space-y-4 rounded-2xl border border-slate-800/70 bg-night-900/50 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  List of degrees or certificates to show on experiences page
                </p>
              </div>
              <label className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-300">
                <input
                  type="checkbox"
                  className="h-4 w-4"
                  checked={sectionsForm.educationsVisible}
                  onChange={handleToggle('educationsVisible')}
                />
            ...
            <div className="space-y-3">
                ...
                <div key={`edu-${idx}`} className="space-y-2">
                  <div className="grid gap-2 md:grid-cols-3">
                    <input
                      ...
                      onChange={handleEditEducation(idx, 'institution')}
                      placeholder="Institution"
                    />
                    <input
                      className={fieldStyle}
                      value={edu.degree ?? ''}
                      onChange={handleEditEducation(idx, 'degree')}
                      placeholder="Degree"
                    />
                    <input
                      className={fieldStyle}
                      value={edu.year ?? ''}
                      onChange={handleEditEducation(idx, 'year')}
                      placeholder="Year"
                    />
                  </div>
                  <div className="flex items-start gap-2">
                    <textarea
                      className={`${fieldStyle} min-h-[64px] w-full`}
                      value={edu.description ?? ''}
                      onChange={handleEditEducation(idx, 'description')}
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (hasContent) {
                          if (
                            !window.confirm(
                              'This education entry contains data. Are you sure you want to remove it?',
                            )
                          )
                            return;
                        }
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={handleAddArrayItem('educationsItems', {
                    institution: '',
                    degree: '',
                    year: '',
                    description: '',
            ...
            <div className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-300">
                <input
                  type="checkbox"
                  className="h-4 w-4"
                  checked={sectionsForm.programmingLanguagesVisible}
                  onChange={handleToggle('programmingLanguagesVisible')}
                />
            ...
            <div className="space-y-2">
                  ...
                  onChange={(e) => {
                    ...
                    setSectionsForm((prev) =>
                      updateSectionsFormField(
                        prev,
                        'programmingLanguagesItems',
                        items as SectionsFormState['programmingLanguagesItems'],
                      ),
                    );
                  }}
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {(sectionsForm.programmingLanguagesItems ?? []).map((lang: string) => (
                  <span
                    key={lang}
                    className="rounded-full border border-slate-800/60 bg-slate-900/70 px-3 py-1 text-xs text-slate-300"
                  >
                    {lang}
                  </span>
                ))}
            ...
            <div className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-300">
                <input
                  type="checkbox"
                  className="h-4 w-4"
                  checked={sectionsForm.languagesSpokenVisible}
                  onChange={handleToggle('languagesSpokenVisible')}
                />
              ...
              <label className="flex flex-col gap-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSectionsForm((prev) =>
                      updateSectionsFormField(prev, 'languagesSpokenItems', [val]),
                    );
                  }}
          ...
          <div className="space-y-4 rounded-2xl border border-slate-800/70 bg-night-900/50 p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Bulleted achievements to show on experiences page
                </p>
              </div>
              <label className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-slate-300">
                <input
                  type="checkbox"
                  className="h-4 w-4"
                  checked={sectionsForm.achievementsVisible}
                  onChange={handleToggle('achievementsVisible')}
                />
            ...
            <div className="space-y-2">
              {(sectionsForm.achievementsItems ?? []).map((a: string, idx: number) => (
                <div key={`ach-${idx}`} className="flex gap-2">
                  <input
                    className={fieldStyle}
                    value={a}
                    onChange={handleEditArrayItem('achievementsItems', idx)}
                  />
                  <button
                    type="button"
                    onClick={handleRemoveArrayItem('achievementsItems', idx)}
                    className="rounded-full border px-3 py-1 text-sm"
                  >
                    Remove
                  </button>
              ...
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={handleAddArrayItem('achievementsItems', '')}
                  className="rounded-full border px-3 py-1 text-sm"
                >
                  Add achievement
                </button>
          ...
          <div className="flex flex-col gap-3 border-t border-slate-800/80 pt-6 sm:flex-row sm:items-center sm:justify-between">
            <div className="text-xs text-slate-500">
              Changes are saved to the server and immediately reflected on the experiences page.
            </div>
            <div className="flex gap-3">
              <button
                type="submit"
                className="inline-flex items-center justify-center rounded-full bg-accent-500 px-5 py-2 text-sm font-semibold text-night-900 shadow-glow transition hover:bg-accent-400"
              >
                Save sections
              </button>
            </div>
