...
beforeAll(() => {
  ...
  if (typeof gIO.IntersectionObserver !== 'function') {
    ...
    gIO.IntersectionObserver =
      MockIntersectionObserver as unknown as GlobalIO['IntersectionObserver'];
  }
});
...
describe('ExperiencesPage', () => {
  ...
  it('omits hidden sections from the side navigation', () => {
    ...
    if (c.sections.programmingLanguages?.visible !== false)
      expectedLabels.push('Programming languages');
    ...
    const matches = navsLabels.some((labels) => {
      ...
      return (
        labels.length === expectedVisibleLabels.length &&
        labels.every((v, i) => v === expectedVisibleLabels[i])
      );
    });
