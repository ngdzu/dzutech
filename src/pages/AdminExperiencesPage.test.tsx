...
describe('AdminExperiencesPage', () => {
  ...
  it('allows editing location and saves it via updateExperiences', async () => {
    ...
    const calledWithRaw = (
      mockContext.updateExperiences as unknown as { mock: { calls: unknown[][] } }
    ).mock.calls[0][0];
  ...
  it('allows adding an experience and saves the new item via updateExperiences', async () => {
    ...
    const initialCount = headingsBefore.filter((h) =>
      /Experience \d+/.test(h.textContent || ''),
    ).length;
    ...
    await waitFor(() => {
      ...
      const newCount = headingsAfter.filter((h) =>
        /Experience \d+/.test(h.textContent || ''),
      ).length;
    ...
    const headingsAfter = screen
      ...
      .filter((h) => /Experience \d+/.test(h.textContent || ''));
    ...
    const titleInput = within(panel).getByPlaceholderText(
    ...
    const textareas = within(panel)
      .getAllByRole('textbox')
      .filter((el) => el.tagName.toLowerCase() === 'textarea') as HTMLTextAreaElement[];
    const descriptionTextarea = textareas[0];
    const skillsInput = within(panel).getByPlaceholderText(
  ...
  it('shows validation error when required fields are missing and does not call updateExperiences', async () => {
    ...
    const skillsInputs = screen.getAllByPlaceholderText(
      'React, TypeScript, GraphQL',
    ) as HTMLInputElement[];
    ...
    await waitFor(() =>
      expect(
        (mockContext.updateExperiences as unknown as { mock: { calls: unknown[][] } }).mock.calls
          .length,
      ).toBe(0),
    );
  });
  ...
  it('saves sections: programming languages and languages spoken via updateSections', async () => {
    ...
    const progInputs = screen.getAllByPlaceholderText(
    ...
    const calledWithRaw = (
      mockContext.updateSections as unknown as { mock: { calls: unknown[][] } }
    ).mock.calls[0][0];
  ...
  it('removing the only experience resets it to an empty experience', async () => {
    ...
    await waitFor(() => {
      ...
      const titleInput = within(newPanel).getByPlaceholderText(
        'Principal Software Engineer',
      ) as HTMLInputElement;
  ...
  it('shows an error message when updateExperiences throws during save', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText((t) => typeof t === 'string' && t.includes('boom'))).toBeTruthy(),
    );
  });
});
