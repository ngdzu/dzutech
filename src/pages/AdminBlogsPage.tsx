...
const AdminBlogsPage = () => {
  ...
  const globalStatus =
    feedback ?? (contextError ? { message: contextError, tone: 'error' as const } : null);
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {loading && (
                <span
                  className={`${badgeStyle} border-slate-700/70 bg-slate-900/70 text-slate-300`}
                >
                  Loading posts…
                </span>
              )}
              ...
              {globalStatus &&
                Array.isArray((globalStatus as { errors?: string[] })?.errors) &&
                (globalStatus as { errors?: string[] }).errors!.length > 0 && (
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (!resp.ok) {
                          ...
                          const message =
                            body && typeof body.message === 'string'
                              ? body.message
                              : resp.statusText;
                          const errors = Array.isArray(body && body.errors)
                            ? body.errors
                            : undefined;
                        ...
                        const count =
                          (body &&
                            (body.count ??
                              (Array.isArray(body.saved) ? body.saved.length : undefined))) ??
        ...
        {sortedPosts.length === 0 && !loading ? (
          ...
          <BlogList
            ...
            onToggleVisibility={async (postId, hidden) => {
              await withControlledAction(postId, hidden ? 'hide' : 'show', () =>
                setPostVisibility(postId, hidden),
              );
            }}
