...
export const BlogList = ({
  ...
  const handleNavigateToDetail = (postId: string) => () => {
    const path = isAdmin
      ? `/admin/blogs/${encodeURIComponent(postId)}`
      : `${basePath}/${encodeURIComponent(postId)}`;
    navigate(path);
  };
  ...
  if (sortedPosts.length === 0 && !loading) {
    return <div className={`${cardStyle} text-center text-sm text-slate-400`}>{emptyMessage}</div>;
  }
    ...
    <>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {pagePosts.map((post) => {
          ...
          return (
            <article key={post.id ?? post.title} className={cardStyle}>
              <div className="space-y-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {isAdmin && post.hidden && (
                    <span
                      className={`${badgeStyle} border-yellow-500/60 bg-yellow-500/10 text-yellow-200`}
                    >
                      Hidden
                    </span>
                  )}
              ...
              <div className="flex flex-wrap gap-2 text-xs text-slate-300">
                {tags.length === 0 ? (
                  ...
                  tags.map((tag) => {
                    ...
                    const tagPath = isAdmin
                      ? `/admin/blogs/tags/${encodedTag}`
